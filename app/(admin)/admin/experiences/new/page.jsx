...
export default function NewExperiencePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Kirim data ke tabel 'experiences' di Supabase
      // Kita akan memetakan input form ke kolom yang ada di tabel
      const { error } = await supabase.from('experiences').insert({
        title: title,          // Input 'Title' akan disimpan di kolom 'role'
        categories: category,    // Input 'Category' akan disimpan di kolom 'company'
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-lg shadow-md">
        ...
        <div>
          <label htmlFor="category" className="block text-sm font-medium text-gray-700">Category / Organization</label>
          <input 
            id="category" 
            type="text" 
