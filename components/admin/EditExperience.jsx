...
export default function EditExperienceForm({ experience }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq('id', experience.id); // Kondisi WHERE untuk memastikan hanya baris yang benar yang di-update
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-lg shadow-md">
      ...
      <div>
        <label htmlFor="title" className="block text-sm font-medium text-gray-700">Role / Position</label>
        <input 
          id="title" 
          type="text" 
      ...
      <div>
        <label htmlFor="categories" className="block text-sm font-medium text-gray-700">Company / Organization</label>
        <input 
          id="categories" 
          type="text" 
